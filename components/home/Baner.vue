    ...
    <div class="search-box">
      <div class="search-item">
        ...
        <div class="search-btn fr">
          <el-button type="small" icon="el-icon-search" @click="goSearch"
            >搜索</el-button
          >
        </div>
      </div>
...
<script>
import { getFullCabinetBase, getSeabulkBaseData } from "~/api";
export default {
  data() {
    return {
      areaPage: [
  ...
  methods: {
    ...
    async getFullCabinetBase() {
      ...
      try {
        let { origins, destinations } = await getFullCabinetBase();
        this.startingPlaceBacOne = origins; //起始地
        this.destinationListBacOneOne = destinations; //目的地
      } catch (error) {
    ...
    async getSeabulkBaseData() {
      try {
        let { origins, destinations } = await getSeabulkBaseData();
        // this.startingPlaceBacTwo = datas.origins
        origins.forEach((el, index) => {
          let item = {
            id: Number(el.id),
            name: el.name
          };
